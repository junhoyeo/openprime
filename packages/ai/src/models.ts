...
export function supportsFastMode<TApi extends Api>(model: Model<TApi>): boolean {
	// Explicit compatibility metadata on an `openai-responses` model decides on its
	// own: a proxy or gateway can advertise support the provider/id table cannot know
	// about, and a models.json override can opt a catalog model out. No catalog
	// `openai-responses` entry carries `compat`, so this only affects declared models.
