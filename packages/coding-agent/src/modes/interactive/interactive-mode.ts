...
export class InteractiveMode {
	...
	private interruptOrClearInput(): void {
		...
		} else if (this.hasActiveRefineCommand()) {
			// Upstream has no /refine, so it never aborts outside a streaming run.
			// A plain abort is right here: there is no active turn whose queued
			// steering should be re-sent.
