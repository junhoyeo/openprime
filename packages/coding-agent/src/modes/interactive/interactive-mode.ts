...
export class InteractiveMode {
	...
	private interruptOrClearInput(): void {
		...
		if (this.isAgentStreaming() || this.hasActiveRefineCommand()) {
			...
			// submit or queued-message edit. Upstream's server-side preservation
			// supersedes this fork's restore-to-editor behaviour on interrupt;
			// the refine-command condition stays, since upstream has no equivalent.
