...
describe("agents-view roster store", () => {
	...
	it("keeps a recovered session attach alive when the roster subscribe fails", async () => {
		...
		const store = new AgentsViewRosterStore();
		// Object.create bypasses the constructor, so nothing the constructor or the field
		// initialisers would have set exists. attach() legitimately reads these, so the fixture
		// supplies them rather than production code defending against an impossible undefined.
