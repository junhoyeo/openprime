...
describe("daemon protocol helpers", () => {
	...
	it("keeps the advertised schema identity synchronized with wire type shapes", () => {
		const source = readFileSync(
			resolve(dirname(fileURLToPath(import.meta.url)), "../src/modes/daemon/daemon-protocol.ts"),
			"utf8",
		);
	...
	it("capability-gates the optional side-question pane id instead of requiring it", () => {
		// Pane grouping is optional metadata, not a precondition for answering: a
		// capable daemon still accepts a start_side_question with no pane id, and an
		// older daemon that silently ignores it is excluded by the capability gate.
