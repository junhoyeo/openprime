...
describe("InteractiveMode working timer", () => {
	...
	function createInitialTimerHarness(snapshot: AgentConnectionSnapshot, turnStartedAt = 1): InitialTimerHarness {
		...
		return Object.assign(Object.create(InteractiveMode.prototype), {
			// Object.create skips the constructor, so `ui` is undefined unless the fixture
			// supplies it, as the other harnesses in this file already do. Upstream's restore
			// path now reaches this.ui.requestRender(); the other 202 call sites are unguarded,
			// so guarding this one alone would be inconsistent and would mask a real undefined.
