...
describe("SettingsManager", () => {
	...
	// The agent-level backoff needs its own ceiling: without one a large maxRetries
	// stops being a retry policy (attempt 30 at a 1s base waits ~6000 days).
	describe("agent-level retry backoff cap", () => {
		it("defaults maxBackoffMs to 60s, honors an explicit 0, and keeps legacy maxDelayMs on the provider cap", () => {
			expect(SettingsManager.create(projectDir, agentDir).getRetrySettings()).toEqual({
				enabled: true,
				maxRetries: 3,
				baseDelayMs: 2000,
				maxBackoffMs: 60000,
			...
			writeSettings(globalPath, { retry: { maxRetries: 30, baseDelayMs: 1000, maxBackoffMs: 0 } });
			expect(SettingsManager.create(projectDir, agentDir).getRetrySettings()).toEqual({
				enabled: true,
				maxRetries: 30,
				baseDelayMs: 1000,
			...
			writeSettings(globalPath, { retry: { maxDelayMs: 5000 } });
			const legacy = SettingsManager.create(projectDir, agentDir);
			expect(legacy.getRetrySettings().maxBackoffMs).toBe(60000);
			expect(legacy.getProviderRetrySettings().maxRetryDelayMs).toBe(5000);
		});
	});
