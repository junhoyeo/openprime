...
/**
 * Tips of foreign history merged into base..HEAD (e.g. an upstream release
 * merged into this fork). Violations those tips already carry are accepted
 * baselines: a merge should not have to fix the whole imported history, only
 * refrain from adding violations of its own. A parent counts as foreign only
 * when it and the base are unrelated in both directions, so a pull request's
 * own branch tip (which always descends from the base) never baselines itself.
 */
function foreignMergeBases(base) {
	if (!base) return [];
	const bases = [];
	for (const line of git(["rev-list", "--merges", "--parents", `${base}..HEAD`], true).split("\n")) {
		const [commit, ...parents] = line.trim().split(/\s+/).filter(Boolean);
		if (!commit) continue;
		for (const parent of parents.slice(1)) {
			if (!isAncestor(parent, base) && !isAncestor(base, parent)) bases.push(parent);
		}
	}
	return bases;
}
...
function maxCounts(...maps) {
	const result = new Map();
	for (const map of maps) for (const [key, value] of map) result.set(key, Math.max(result.get(key) ?? 0, value));
	return result;
}

const base = resolveBase();
const foreignBases = foreignMergeBases(base);
const failures = [];
for (const path of changedTestFiles(base)) {
	const current = scan(readFileSync(resolve(root, path), "utf8"), path);
	const baselines = [base, ...foreignBases].map((ref) => (ref ? git(["show", `${ref}:${path}`], true) : ""));
	const allowed = maxCounts(...baselines.map((content) => counts(content ? scan(content, path) : [])));
	const seen = new Map();
